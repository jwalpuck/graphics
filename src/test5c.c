...
int main(int argc, char *argv[]) {
  ...
  //Our code....in the middle of the street
  int count;
  ...
  //if( argc > 1 ) {
    ...
    point_set( &(view.vrp), 3*alpha, 2*alpha, -2*alpha - (1.0-alpha)*3,0);
    //}
  /* else { */
  ...
  if(count<10){
  char s1[] = "/export/home/vedwards/Desktop/Graphics/images/3DCube/cube00";
  char *s2 = malloc(sizeof(char) * 3);
  snprintf(s2, sizeof(char) * 3, "%d", count);
  char s3[] = ".ppm";
  strcat(s1, s2);
  strcat(s1, s3);
  ...
  free(s2);
    ...
    char s1[] = "/export/home/vedwards/Desktop/Graphics/images/3DCube/cube0";
  char *s2 = malloc(sizeof(char) * 3);
  snprintf(s2, sizeof(char) * 3, "%d", count);
  char s3[] = ".ppm";
  strcat(s1, s2);
