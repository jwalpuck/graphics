/*
  This is the frame work for a scanline fill algorithm. This will allow us to fill a polygon!

  Bruce A. Maxwell
  Added to by: Jack Walpuck and Torrie Edwards
  Fall 2014
...
static Edge *makeEdgeRec( Point start, Point end, Image *src)
{
	...
	******/
	...
	Vydec = edge->y0-(float)((int)(edge->y0));
	if(Vydec < 0.5){
	  adjust = 0.5-(Vydec);
        }else{
	  adjust = 1.5-(Vydec);
        }
	edge->xIntersect =(int)(edge->x0+adjust*edge->dxPerScan);
	// Scanlines go through the middle of pixels
	// Move the edge to the first scanline it crosses
	//BRUCE COMMENT...
	edge->x0 = edge->xIntersect +0.5; 
	
	// adjust if the edge starts above the image
	// move the intersections down to scanline zero
	// if edge->y0 < 0
	if(edge->y0<0){
	  //   update xIntersect Bruce Comment... This is not right
	  edge->xIntersect = (int)(edge->x0+0.5);
	  //   update y0 Bruce Comment
	  //edge->y0 = edge->y0+(-1*edge->y0); // this will set the y0 value to 0 if y0<0. 
	  //   update x0 Bruce Comment
	  //edge->x0 = (edge->y1-edge->y0)/edge->dxPerScan + edge->x0;//dxPerScan = m
	  //   update yStart
	  edge->yStart = edge->y0;//same thing as y0 but this is an int
	}
	...
	/* } */
	//BRUCE COMMENT...
	if(edge->dxPerScan < 0 || edge->dxPerScan>0 ){
...
static LinkedList *setupEdgeList( Polygon *p, Image *src) {
	...
	for(i=0;i<p->nVertex;i++) {
		...
		// if it is not a horizontal linex
...
static void fillScan( int scan, LinkedList *active, Image *src, Color c ) {
  ...
  while(p1) {
    ...
    start =  p1->xIntersect;
    // clip, check if outside of the bounds of the image >0 or 0, to the left side of the image
    start = start<0?0:start;
    // identify the ending column
    end = p2->xIntersect;
    // clip to the right side of the image
    end = end>=src->cols?src->cols-1:end;
